...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query(value = "SELECT m FROM Message m JOIN m.fromId f WHERE m.toId = :toId AND m.deletedByFrom = false AND m.content LIKE %:content% AND f.deleted = false")
    List<Message> findMessages(Member toId, String content);
}
